...
def render_lowlight_enhance(
    ...
    """
    Deterministic low-light enhancement for rendered images.

    Args:
        image: torch.Tensor (H,W,3) in [0,1]
        enable: bool flag to enable enhancement
        target_mean: target brightness
        max_scale: max brightness scaling
        gamma_base: base gamma
        gamma_slope: adaptive gamma slope
        gamma_min/max: gamma range
        contrast: contrast strength
        saturation: color saturation strength
    ...
    # 2 Adaptive gamma
    # ----------------------------
    gamma = gamma_base + gamma_slope * (0.5 - brightness)
    gamma = torch.clamp(
        torch.tensor(gamma, device=image.device, dtype=image.dtype),
        min=gamma_min,
        max=gamma_max
    )
...
def readCamerasFromTransforms(path, transformsfile, white_background, extension=".png"):
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        skip = 1 #if transformsfile == 'transforms_test.json' else 1
        ...
        for idx, frame in tqdm(enumerate(frames)):
            ...
            mask = norm_data[:, :, 3:4]
            if skip == 1000:    #skip == 1000 is a hack to load depth maps only for the Blender dataset, which has 1000 frames
            ...
            cam_infos.append(CameraInfo(uid=idx, R=R, T=T, FovY=FovY, FovX=FovX, image=image, image_path=image_path,
                                        ...
                                        depth_image=depth_image, mask=mask,bounds=[0.1, 10.0]))
    return cam_infos
